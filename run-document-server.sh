...
if [[ $RULE =~ ',' ]]; then
  ...
  for rule in ${rules[@]}; do
    #echo $rule
    my_array[$length]='{"address": "'$rule'", "allowed": true}'
    let length++
  done
else
  my_array[1]='{"address": "'$RULE'", "allowed": true}'
  let length++
fi

my_array[$length]='{"address": "*", "allowed": false}'
...
#echo $RULE
DEFAULT_RULE='{"address": "127.0.0.1", "allowed": false}'

sed -i "s|${DEFAULT_RULE}|${result}|g" /etc/onlyoffice/documentserver/default.json
